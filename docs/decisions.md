...
3. **Encoders 100% em Aether:** os formatos são skills na stdlib (`std.serde`): `skill Json : Serializable { fun toJson(): String }` e `skill Yaml : Serializable { fun toYaml(): String }`, compostas com `+`. Seus corpos são `.ae` puro e apenas percorrem a lista de `serdeFields()`. Novos formatos (Toml, XML, binário) são skills novas sem tocar o compilador.
4. **Valores heterogêneos via contract `SerdeValue`:** como a lista de campos é heterogênea, cada valor é encaixotado em um box std que implementa `SerdeValue` (`IntValue`, `FloatValue`, `BoolValue`, `StringValue`, `ObjectValue`, `ListValue`). O contract expõe `kind()` e acessores; `ObjectValue`/`ListValue` permitem recursão.
5. **Regra de campos:** só entram na lista gerada campos de tipos serializáveis — primitivos (`Int`, `Float`, `Bool`, `String`), `type`s que implementam `Serializable` (recursivo) e `List<T>` com `T` serializável. Campos de qualquer outro tipo são **ignorados silenciosamente**. Campos `Map<K,V>` ficam para uma fase futura.
