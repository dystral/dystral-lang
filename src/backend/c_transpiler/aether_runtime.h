...
static inline AetherString* AetherString_new(const char* literal) {
...
static inline AetherString* AetherString_fromInt(int val) {
    char buf[32];
    sprintf(buf, "%d", val);
    return AetherString_new(buf);
}

static inline AetherString* AetherString_fromBool(bool val) {
...
static inline AetherString* _AetherString_plus(AetherString* a, AetherString* b) {
...
static inline void AetherString_print(void* ptr) {
